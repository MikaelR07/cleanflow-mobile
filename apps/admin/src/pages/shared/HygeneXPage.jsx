...
export default function HygeneXPage() {
  ...
  const businessPrompts = [
    ...
    "How can I improve my listing match score?",
    "Show my pending purchase orders",
    "Analyze marketplace demand for plastic"
  ];
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-4">
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-1">{metrics?.estates || 0} <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">Estates</span></div>
            <p className="text-xs text-slate-500 dark:text-slate-400">Actively monitored right now.</p>
          </div>
          
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="text-2xl font-bold text-primary dark:text-primary-light mb-1">{metrics?.activeAgents || 0} <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">Agents</span></div>
              ...
              <div className="relative w-12 h-12">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                  ...
                  <circle 
                    ...
                    strokeDashoffset={`${2 * Math.PI * 45 * (1 - (metrics?.segregationRate || 0) / 100)}`} 
                  />
                </svg>
              </div>
              <div>
                <div className="text-xl font-bold text-slate-800 dark:text-slate-100">{metrics?.segregationRate || 0}%</div>
