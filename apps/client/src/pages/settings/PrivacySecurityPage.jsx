      ...
      {authStage === 'view' ? (
        ...
        <form onSubmit={handleChangePin} className="card p-5 space-y-5 animate-slide-up border-t-4 border-t-blue-500">
           <div>
              ...
              <input type="password" required minLength={8} value={pins.current} onChange={(e) => setPins({...pins, current: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500/50" placeholder="••••••••" />
           </div>
           <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wider">New Password</label>
              <input type="password" required minLength={8} value={pins.new} onChange={(e) => setPins({...pins, new: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500/50" placeholder="Minimum 8 characters" />
           </div>
           <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wider">Confirm New Password</label>
              <input type="password" required minLength={8} value={pins.confirm} onChange={(e) => setPins({...pins, confirm: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500/50" placeholder="Repeat new password" />
           </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-6">
          <div className="bg-white dark:bg-slate-900 w-full max-w-[320px] rounded-[28px] overflow-hidden shadow-2xl animate-scale-in border border-slate-100 dark:border-slate-800">
            <div className="bg-rose-500 p-6 flex flex-col items-center gap-3 text-white">
              ...
              <h2 className="text-lg font-black uppercase tracking-tighter">Delete Account?</h2>
            </div>
            <div className="p-6 space-y-4">
              <p className="text-xs text-center text-slate-500 dark:text-slate-400 leading-relaxed">
                This will <span className="font-bold text-rose-500 underline decoration-rose-500/30">wipe all records</span>, scheduled bookings, and reward points permanently.
              </p>
