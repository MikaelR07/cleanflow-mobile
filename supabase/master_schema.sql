...
BEGIN
  SELECT json_build_object(
    ...
    'activeAgents',       (SELECT count(*) FROM public.profiles WHERE role = 'agent'),
    'totalBusinesses',    (SELECT count(*) FROM public.profiles WHERE role = 'business'),
    'totalVerified',      (SELECT count(*) FROM public.profiles WHERE is_verified = true),
    'totalWeight',        COALESCE((SELECT sum(weight_kg) FROM public.bookings WHERE status = 'completed'), 0),
